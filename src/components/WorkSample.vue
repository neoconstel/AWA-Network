<template>
    ...
    <div v-if="this.isMounted" class="relative" :id="'work-' + this.work.id" @mouseenter="toggleStats"
        ...
        <div class="relative bg-violet-600 aspect-square">
            <a v-if="showDelete && this.workType == this.workTypes.projects" @click.prevent="storeWork" href="">
                <img class="absolute top-0 right-0 bg-red-500 opacity-60" src="/static/icons/iconmonstr-x-mark-thin.svg"
                    alt="" data-twe-toggle="modal" data-twe-target="#deleteProjectModal">
            </a>
        ...
        <div :class="['text-gray-800 dark:text-gray-200', infoBgCol]">
            <p class="text-center overflow-hidden whitespace-nowrap text-ellipsis px-1"><b>{{ work.title }}</b></p>
            <p class="overflow-hidden whitespace-nowrap text-ellipsis px-1"><svg name="user-icon"
                    class="inline w-3 mx-2 fill-gray-800 dark:fill-gray-200" xmlns="http://www.w3.org/2000/svg"
                ...
                <RouterLink v-if="work.id" @click="storeWork" :to="`/artistPortfolio/${work.artist.user.username}`">
                    <span
                        class="inline text-cyan-700 dark:text-cyan-500 hover:text-pink-700 dark:hover:text-gray-100">{{
