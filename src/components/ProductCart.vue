<template>
    <div v-if="licenses.length" v-for="(license, index) in licenses" class="border-2 border-gray-500 m-3 p-5"
        ...
        <!-- if license has been added -->
        <v-btn v-if="license.added" class="mt-5" block variant="outlined">
            Added to Library
        </v-btn>
        <!-- else if license not added and has a price -->
        ...
        <!-- else if license not added but is free -->
        <v-btn v-else class="mt-5" block variant="outlined">
            <svg v-if="this.dataStore.user.id" class="fill-gray-800 dark:fill-gray-200" width="24" height="24"
            ...
            <span v-if="this.dataStore.user.id" @click="libraryAdd(product, license)">Add to library</span>
            <span v-else>No login needed</span>
        </v-btn>
    </div>
</template>
